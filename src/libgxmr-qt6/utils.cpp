...
bool CompareNames(const QString& fileName1, const QString& fileName2) 
{
    ...
    while ((pos = fileName1.indexOf(rd, pos, &m1)) != -1) {
        auto inc = m1.capturedLength();
        auto id1 = fileName1.mid(pos, inc);

        QRegularExpressionMatch m2;
        auto pos2 = fileName2.indexOf(rd, pos, &m2);
        if (pos == pos2) {
            auto id2 = fileName2.mid(pos, m2.capturedLength());
...
{
    QScreen *screen = w->screen() ? w->screen() : QGuiApplication::primaryScreen();
